...
**`Feed` は自動で付けない。** これはサイトを見た人が読む一覧で、About の
「フィードの取得状況」から
`is:issue is:open label:Feed` へリンクしている（`pages/about.vue`）。
配信者が自分の番組の状況を確かめに来る場所なので、開発の作業 issue が
混ざると読めなくなる。

付けるのは**訪問者に知らせたい既知の問題だけ**。フィードに関する内容でも、
直す作業そのものを書いた issue には付けない。

いま付いているものは、どちらも継続的に追記していくまとめ。書き方はこれに倣う。

- #5 登録できなかった番組（配信元ごとに、フィードの不備と参照先を並べたもの）
- #52 Requestがあったが登録見合わせたものとその理由（表で1行ずつ）

閉じると一覧から消えるので、役目が続くうちは開けたままにする。

他のラベル（`enhancement`・`UI`・`Feature Request`・`site-health`・
`register-request`）は開発側のもので、公開の一覧には出ていない。
...
**このリポジトリでは `git commit -m` に複数行をそのまま渡す。ファイルは作らない。**
`-m` の中で改行してよく、1行目が見出し、空行を挟んで本文になる。ここは
作業ディレクトリなので bash から git が使えるため、メッセージを中継する
ファイルは要らない。作ると `rm` がサンドボックスに弾かれ、消す手間だけが残る。
...
ファイルが要るのは、bash から git そのものが使えない場所（`~/.claude/` 配下の
Skill など）で、ユーザーに手で実行してもらうときだけ。そのとき `git commit -F`
に渡すファイルは、`$TMPDIR/msg.txt` のような汎用名にしない。
