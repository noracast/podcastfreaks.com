...
Node は `.node-version` に固定。Nuxt 2 のままだが 24 系で動く。webpack 4 は
Node 17 以降（OpenSSL 3）で壊れるとよく言われるが、Nuxt 2.18 は回避済みで、
Node 20 と Node 24 でそれぞれビルドした `dist` を比べたところ、JS チャンクは
1バイトも違わなかった（差はビルド時刻だけ）。上げられない理由は無い。
