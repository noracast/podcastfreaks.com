...
describe('toChannel', () => {
  ...
  // Apple に id の違う項目が2つ並んでいても、feedUrl が同じなら配信元は1つ。
  // /request に同じ行が2つ並んでいた（ひまじんプログラマーの週末エンジニアリングレッスン）
  it('同じフィードの番組は1つにまとめ、先に出たほうを残す', () => {
    const channels = toChannels({ results: [
      { ...result, collectionName: '先', collectionViewUrl: 'https://podcasts.apple.com/jp/podcast/x/id1601084785' },
      { ...result, collectionName: '後', collectionViewUrl: 'https://podcasts.apple.com/jp/podcast/x/id1862003490' }
    ] })
    expect(channels).toHaveLength(1)
    expect(channels[0].title).toBe('先')
  })
})
