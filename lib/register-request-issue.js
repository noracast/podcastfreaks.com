...
//
// 登録リクエストのページはサーバーを持たない（issue #228）。送信先は GitHub の
...
export function issueBody({ channel = {}, source = {} } = {}) {
  ...
  // フィードが分からないまま送られることがある（Apple に載っていない番組や、
  // 検索で見つからなかった場合）。そのときは、書き足してほしいことを
  // 本文に添える。#231 の検証はフィードが無いと始められない
  const askFeed = channel.feed
    ? []
    : ['', '> フィード（RSS）の URL が分かれば、上の「フィード」に書き足してください。', '> 分からない場合は、番組のページの URL だけでも構いません。']
  ...
  return [
    ...
    '',
    'このリクエストは https://podcastfreaks.com/request/ から作りました。',
