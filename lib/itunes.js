...
//
// もうひとつ、同じ番組が2件返ることがある。Apple 側に id の違う項目が並んでいても、
// feedUrl が同じなら配信元は1つ（ひまじんプログラマーの週末エンジニアリング
// レッスンが id1601084785 と id1862003490 の2件で返り、/request に同じ行が
// 並んでいた）。ここで見ているのはフィードなので、先に出たほうを残す
export default function toChannels(response) {
  const results = (response && response.results) || []
  const seen = new Set()
  return results.map(toChannel).filter(channel => {
    if(!channel || seen.has(channel.feed)) return false
    seen.add(channel.feed)
    return true
  })
}
