...
//
// /request は「Spotify を開いていた」「Apple のページを見ていた」「番組サイトに
...
import { normalizeTitle } from './registered-match.js'
...
// 番組サイトのホスト名から、番組名になりそうな部分を取る
// （fukabori.fm → fukabori、www.ossan.fm → ossan）。
//
// タイトルが使えないとき（回のタイトルだけが入っているなど）の最後の
// 手がかり。ただし**これで引いた結果はそのまま出せない。** iTunes の検索は
// 説明文や作者欄にも当たるため、example.com のようなありふれた名前だと
// 無関係な番組が並ぶ。呼ぶ側で、番組名にこの語を含むものだけを残すこと
// （pages/request.vue の findViaHost）。
//
// 最後のひとつ（TLD）を落として、その手前を使う。co.jp のような
// 二段の TLD は正しく取れないが、番組サイトでは滅多に無い
export function hostLabel(url) {
  const { kind, host } = detectSource(url)
  if(kind !== 'site' || !host) return ''
  const parts = host.split('.').filter(Boolean)
  if(parts.length < 2) return ''
  return parts[parts.length - 2]
}

// フィードの URL でよく使われる語。これ自体は番組名の手がかりにならない
const FEED_PATH_NOISE = /^(feed|feeds|rss|atom|xml|podcast|podcasts|index|show|shows|s)$/i

// 中身を持たない id（anchor.fm/s/107747d74、数字だけの回 id）。
// 数字を含む16進の並びだけを弾く。decade のような語まで落とさないため
const ID_LIKE = /^(?=.*\d)[0-9a-f-]{6,}$/i

// フィードの URL から、番組名になりそうな部分を取る
// （rss.art19.com/fukabori → fukabori）。
//
// ホスト名が配信基盤のもの（art19・anchor・soundcloud）になる形では、
// hostLabel が使えない。iTunes には URL で引く方法が無いので、パスの
// 末尾から番組名らしい語を探す。
//
// hostLabel と同じく、**これで引いた結果はそのまま出せない。**
// 呼ぶ側で startsWithHostLabel を通して絞ること
export function pathLabel(url) {
  let pathname = ''
  try {
  ...
  // 後ろから見る。feed.xml のような末尾を飛ばすと、その手前に番組名がある
  for(let i = parts.length - 1; i >= 0; i--) {
    const part = decodeURIComponent(parts[i]).replace(/\.(xml|rss|json|php)$/i, '')
    if(!part || FEED_PATH_NOISE.test(part) || ID_LIKE.test(part)) continue
    return part
  }
  return ''
}

// 入れられた URL が、番組のページではなくフィードそのものに見えるか。
// 見つからなかったときに、これをフィードとして送るかどうかの判断に使う。
//
// 当てにいくのではなく、**明らかにフィードの形のものだけ**を通す。
// 番組のページを誤ってフィードとして送ると、こちらで気づけない
export function looksLikeFeed(url) {
  ...
  if(/\.(xml|rss)$/.test(pathname)) return true
  return /(^|\/)(feed|feeds|rss|atom)\/?$/.test(pathname)
}

// ホスト名で引いた結果を絞る。**番組名がその語で始まるものだけ**を通す。
//
// 「含む」だと緩すぎて、example で引いたときに「Lead By Example with Bob
// Myers」のような番組が残ってしまう。番組サイトのホスト名は番組名そのもの
// （fukabori.fm → fukabori.fm、ossan.fm → Ossan.fm）なので、先頭で見る
export function startsWithHostLabel(title, label) {
  if(!title || !label) return false
  return normalizeTitle(title).startsWith(normalizeTitle(label))
}
