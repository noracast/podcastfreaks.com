...
describe('issueBody（フィードが分からないまま送るとき）', () => {
  // 見つからなかった番組も、ここで終わらせずに送れるようにしている。
  // #231 の検証はフィードが無いと始められないので、書き足しを促す
  it('フィードが空なら、書き足してほしいと添える', () => {
    expect(issueBody({ channel: { title: 'テスト番組' } })).toContain('> フィード（RSS）の URL が分かれば')
  })

  it('フィードが分かっているときは添えない', () => {
    expect(issueBody({ channel })).not.toContain('> フィード（RSS）の URL が分かれば')
  })
})
