...
import searchTerms, { appleIdFromUrl, cleanTitle, detectSource, hostLabel, looksLikeFeed, pathLabel, startsWithHostLabel } from './podcast-source.js'
...
describe('pathLabel', () => {
  // ホスト名が配信基盤のものだと hostLabel が使えない。パスの末尾を見る
  it('フィードの URL から番組名になりそうな部分を取る', () => {
    expect(pathLabel('https://rss.art19.com/fukabori')).toBe('fukabori')
    expect(pathLabel('https://feeds.feedburner.com/3jumper')).toBe('3jumper')
  })

  it('feed や rss のような語は飛ばす', () => {
    expect(pathLabel('https://fukabori.fm/feed.xml')).toBe('')
    expect(pathLabel('https://example.com/podcast/rss')).toBe('')
  })

  // anchor.fm/s/<hex> の hex は番組名と関係が無い
  it('id に見えるものは手がかりにしない', () => {
    expect(pathLabel('https://anchor.fm/s/107747d74/podcast/rss')).toBe('')
  })

  it('URL でなければ空', () => {
    expect(pathLabel('')).toBe('')
    expect(pathLabel('fukabori')).toBe('')
  })
})

describe('looksLikeFeed', () => {
  it('明らかにフィードの形のものだけを通す', () => {
    expect(looksLikeFeed('https://fukabori.fm/feed.xml')).toBe(true)
    expect(looksLikeFeed('https://anchor.fm/s/107747d74/podcast/rss')).toBe(true)
    expect(looksLikeFeed('https://example.com/feed/')).toBe(true)
  })

  it('番組のページは通さない', () => {
    expect(looksLikeFeed('https://open.spotify.com/show/abc')).toBe(false)
    expect(looksLikeFeed('https://rss.art19.com/fukabori')).toBe(false)
