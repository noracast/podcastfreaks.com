...
日次ビルドが失敗し続けても誰も気づかない状態が長く続いたため、気づく仕組みを入れています。

ビルドで見つかった問題は**すべて** `build_info.json` に記録し、https://podcastfreaks.com/errors/ に出します。フィードを直せる方に見ていただくためです。

| 種類 | 内容 |
|---|---|
| 取得できなかった番組 | フィードにアクセスできない、エピソードが無い、RSS でも Atom でもない |
| 音声を持たない番組 | 記事用のフィードを登録している、ドメインが第三者に取得された、など |
| 収録時間を読み取れない番組 | `itunes:duration` が0、または書式が不正 |
| カバー画像を取得できない番組 | 画像のURLが切れている、403 や 404 を返す |
| XML が壊れている番組 | 実体参照になっていない `&` を書き換えて読み込んだ |
| `data/rss.json` の問題 | 登録の重複など |

一方、**issue を立てるのはこちらで扱いを決める必要があるものだけ**です。

- ビルドが36時間以上更新されていない（日次ビルドが止まっている）
- フィードを取得できない番組がある
- 音声を持たない番組がある（登録を外すかどうかの判断が要る）

収録時間やカバー画像はフィード側の不備でこちらでは直せず、毎日 issue が立ったままになると肝心の変化に気づけなくなるため、issue には参考として並べるだけにしています。
